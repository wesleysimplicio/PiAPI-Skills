# RELEASE — `PiAPI-Skills`

Processo para cortar uma release de `PiAPI-Skills` (`TBD`, stack `dotnet`). Releases são tagueadas, automatizadas via GitHub Actions e reversíveis. Dono do processo: `Wesley Simplicio`.
  ...
  deploy:
    ...
    steps:
      ...
      - name: Rollout production
        run: <comando rollout>
      - name: Smoke test
        run: npm run test:smoke -- --baseUrl=https://PiAPI-Skills.io
      - name: Notify
...
- Workflow separado `deploy-rc.yml` envia pra ambiente `rc.PiAPI-Skills.io`.
