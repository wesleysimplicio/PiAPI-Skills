...
Adote git-flow apenas se o projeto exige releases longas com hardening, ex: software embarcado, app mobile com review de loja, produto SaaS com janela de manutenção rigorosa. Branches: `main` (produção), `develop` (integração), `feature/*`, `release/*`, `hotfix/*`. Para `PiAPI-Skills` web/SaaS o padrão é trunk-based.
...
|----------|------------|---------|-----|
| `dev` | qualquer PR | abrir/atualizar PR | preview-<pr>.PiAPI-Skills.dev |
| `staging` | `main` | merge em main | staging.PiAPI-Skills.io |
| `production` | tag `vX.Y.Z` | tag push manual | PiAPI-Skills.io |
...
Configuração em GitHub do repo `PiAPI-Skills`:
